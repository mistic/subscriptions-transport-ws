  ...
  it('should send INIT message first, then the SUBSCRIPTION_START message', (done) => {
    ...
    wsServer.on('connection', (connection: any) => {
      connection.on('message', (message: any) => {
        ...
        if (parsedMessage.type === MessageTypes.SUBSCRIPTION_START) {
  ...
  it('should allow both data and errors on SUBSCRIPTION_DATA', (done) => {
    wsServer.on('connection', (connection: any) => {
      connection.on('message', (message: any) => {
        ...
        if (parsedMessage.type === MessageTypes.SUBSCRIPTION_START) {
          connection.send(JSON.stringify({type: MessageTypes.SUBSCRIPTION_SUCCESS, id: parsedMessage.id}), () => {
            const dataMessage = {
              type: MessageTypes.SUBSCRIPTION_DATA,
              id: parsedMessage.id,
              payload: {
                data: {
                  some: 'data',
      ...
      connection.on('message', (message: any) => {
        ...
        if (parsedMessage.type === MessageTypes.SUBSCRIPTION_START) {
          connection.send(JSON.stringify({
            type: MessageTypes.SUBSCRIPTION_FAIL,
            id: parsedMessage.id,
            payload,
          }));
        }
